...
class Settings(BaseSettings):
    ...
    cpu_pending_jobs: int = Field(default=256, ge=1)
    asr_concurrency: int = Field(default=30, ge=1)
    asr_max_waiters: int = Field(default=30, ge=0)
    ...
    tts_idle_timeout_seconds: float = Field(default=5.0, gt=0, allow_inf_nan=False)
    tts_drain_timeout_seconds: float = Field(default=0.01, gt=0, allow_inf_nan=False)
    ...
    turn_end_inference_timeout_ms: int = Field(default=500, gt=0)
    ...
    def validate_turn_end(self):
        ...
        if self.turn_end_min_silence_ms > self.turn_end_max_silence_ms:
            raise ValueError("turn-end min silence must not exceed max silence")
